    ...
    <div>
      ...
      <div className="mx-auto max-w-5xl px-4 py-10 sm:px-6 space-y-6 text-slate-900 dark:text-slate-100">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">
              {ticket.visitorEmail || "No email on file"}
            </p>
          </div>
                  ...
                  <div className="flex flex-wrap gap-2 text-xs text-slate-600 dark:text-slate-300">
                    {(msg as any).metadata.attachments.map((file: any, idx: number) => (
                      <a
                        key={`${msg.id}-att-${idx}`}
                        href={file.url}
                        className="inline-flex items-center gap-1 rounded-full border border-slate-200 bg-white px-3 py-1 font-semibold text-indigo-700 hover:text-indigo-800 dark:border-slate-700 dark:bg-slate-900 dark:text-indigo-300 dark:hover:text-indigo-200"
                        target="_blank"
                        rel="noreferrer"
                      >
                        Attachment {idx + 1}
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm space-y-4 dark:border-slate-700 dark:bg-slate-900">
          ...
          <form action={`/api/admin/support/tickets/${ticket.id}/status`} method="post" className="flex items-center gap-3 text-sm">
            <label className="font-semibold text-slate-700 dark:text-slate-200">Status</label>
            <select
              name="status"
              defaultValue={ticket.status}
              className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700 shadow-sm focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-100 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 dark:focus:ring-indigo-500/30"
            >
